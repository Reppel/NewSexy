...
<head lang="en">
    <meta charset="UTF-8">
    <title>New Sexy - Search</title>
    <link rel="stylesheet" href="css/search.css">
    <script src="javascript/lib/jquery-2.1.3.min.js"></script>
    <script type="text/javascript" src="javascript/search.js" defer="defer" charset="utf-8"></script>
</head>
<body>
<div id="ask_block">
...
<div class="wrapper">
    ...
    <div id="body">
        <div id="main_body">
            <p id="search_result_text"><span id="key_word">"${key}"</span>的搜索结果：</p>
            <div id="search_result_block">
                <div id="result_nav_bar">
                    <ul id="result_tab_list">
                        <li class="result_tab active" id="result_tab_question">问题</li>
                        <li class="result_tab" id="result_tab_user">用户</li>
                    </ul>
                </div>

                <div id="result_tab_panel">
                    <div class="result_tab_content active" id="result_question">

                        <c:forEach items="${question_list}" var="question" varStatus="index">
                            <c:if test="${not empty question_list}">
                                <div class="question_item_block">
                                    <a href="questionServlet?questionID=${question.questionID}"><p class="question_item_title">${question.questionTitle}</p></a>
                                    <div class="question_item_footer">
                                        <a href="#"><span class="ask_username">${question.username}</span></a>
                                        <span class="ask_text">提问于</span>
                                        <span class="ask_time">${question.questionDate}</span>
                                    </div>
                                </div>
                            </c:if>
                        </c:forEach>
                        <c:if test="${empty question_list}">
                            <span class="no_key">:( 没有关于"${key}"的问题</span>
                        </c:if>
                    </div>
                    
                    <div class="result_tab_content" id="result_user" >

                        <c:forEach items="${user_list}" var="user" varStatus="index">
                            <c:if test="${not empty user_list}">
                                <div class="user_item_block">
                                    <a href="userProfileServlet?userID=${user.userID}"><img src="${user.headURL}" class="user_item_head"></a>
                                    <a href="userProfileServlet?userID=${user.userID}" class="user_item_username">${user.username}</a><br/>
                                    <span class="user_item_sign">${user.sign}</span>
                                    <div class="follow_follower_info">
                                        <div class="ff_info_block follow_info">
                                            <span>关注</span><br/>
                                            <span class="people_number follow_number">${user.followingCount}</span><br/>
                                        </div>
                                        <div class="ff_info_block follower_info">
                                            <span>关注者</span><br/>
                                            <span class="people_number follower_number">${user.followerCount}</span><br/>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <c:if test="${empty user_list}">
                            <span class="no_key">:( 没有关于"${key}"的用户</span>
                        </c:if>
                    </div>
                </div>
