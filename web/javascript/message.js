...
var received_message_count=document.getElementsByClassName("received").length;
for(var i=0;i<received_message_count;i++){
    document.getElementsByClassName("message_dialog_text")[i].setAttribute("onclick","click_not_read("+i+")");
}


function click_not_read(index){
   var read_status=document.getElementsByClassName("read_status")[index].value;
    var message_id=document.getElementsByClassName("message_id")[index].value;
    if(read_status==0){
       mark_read(index,message_id);
   }
}

function mark_read(index,message_id){
    $.ajax({
        type:'POST',
        url:"/markReadServlet",
        ...
        success:
            function(data){
                document.getElementsByClassName("message_dialog_text")[index].innerHTML="[已读]";
